...
class TestDatabrief(unittest.TestCase):
    ...
    def test_large_number_of_booleans(self):
        @dataclass
        class ManyBools:
            b1: bool
            b2: bool
        ...
        original = ManyBools(b1=True, b2=False, b3=True, b4=False, b5=True, b6=False, b7=True, b8=False, b9=True)
        dumped = dump(original)
        loaded = load(dumped, ManyBools)
        self.assertEqual(original, loaded)

    def test_all_supported_field_types(self):
        original = TestDataMoreFields(a=1, b=2.0, c=True, d=-1, e=-2.0, f=False)
        dumped = dump(original)
